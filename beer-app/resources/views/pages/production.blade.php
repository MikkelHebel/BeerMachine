    ...
    <form action="{{ route('send.command') }}" method="POST" class="production-form" id="production-form">
        ...
        <div class="buttons">
            ...
            <form action="{{ route('send.command') }}" method="POST">
                ...
                <button type="submit" class="button btnStart">Start</button>
            </form>

            <form action="{{ route('send.command') }}" method="POST">
                @csrf
                <input type="hidden" name="type" value="stop">
                <button type="submit" class="button btnStop">Stop</button>
            </form>

            <form action="{{ route('send.command') }}" method="POST">
                @csrf
                <input type="hidden" name="type" value="reset">
                <button type="submit" class="button btnReset">Reset</button>
            </form>
        </div>
